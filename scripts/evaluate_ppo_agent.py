...
class PairedStats:
    ...
    ppo_payments: list[int] = field(default_factory=list)
    deltas: list[int] = field(default_factory=list)

    def add(self, heuristic_payment: int, ppo_payment: int) -> None:
        self.heuristic_payments.append(heuristic_payment)
        self.ppo_payments.append(ppo_payment)
        self.deltas.append(ppo_payment - heuristic_payment)
...
def run_evaluation(
    *,
    ...
    ppo = PPOCardPlayAgent.from_checkpoint(checkpoint, device=device, stochastic=False)
    ...
    for game_index in range(games):
        ...
        ppo_result = run_variant(
            focal_agent=ppo,
            game_index=game_index,
            master_seed=seed,
        )

        if heuristic_result.contract != ppo_result.contract:
            raise RuntimeError(
                "Paired runs produced different contracts despite identical bidding: "
                f"{heuristic_result.contract} != {ppo_result.contract}."
            )

        heuristic_payment = heuristic_result.payments[focal_seat]
        ppo_payment = ppo_result.payments[focal_seat]
        overall.add(heuristic_payment, ppo_payment)
        ...
        ppo_wins += int(focal_seat in ppo_result.game_result.winner_players)

        if ppo_payment > heuristic_payment:
            ppo_better += 1
        elif ppo_payment == heuristic_payment:
            equal += 1
        else:
            heuristic_better += 1
    ...
    ppo_mean = statistics.fmean(overall.ppo_payments)
    delta = statistics.fmean(overall.deltas)
    ci_low, ci_high = confidence_interval_95(overall.deltas)

    print("\nPPOCardPlayAgent vs HeuristicAgent — paired card-play evaluation")
    print("=" * 88)
    ...
    print(f"PPO mean payment:           {ppo_mean:+.3f}")
    print(f"Mean PPO improvement:       {delta:+.3f}")
    print(f"95% CI paired improvement:  [{ci_low:+.3f}, {ci_high:+.3f}]")
    print(f"Heuristic win rate:         {heuristic_wins / games:.2%}")
    print(f"PPO win rate:               {ppo_wins / games:.2%}")

    print("\nPer-deal payment comparison")
    print("-" * 88)
    print(f"PPO better:                 {ppo_better:>8,}")
    ...
    for game_type in GameType:
        ...
        print(
            ...
            f"ppo={statistics.fmean(stats.ppo_payments):>+8.3f}  "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare a PPO card-play policy with HeuristicAgent on paired deals. "
            "Both use the same heuristic bidding policy."
        )
    )
    parser.add_argument(
        "--checkpoint",
